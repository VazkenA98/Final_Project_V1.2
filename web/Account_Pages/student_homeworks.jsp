  ...
  <div id="wrapper">
    ...
    <div id="content-wrapper" class="d-flex flex-column">
      ...
      <div id="content">
          ...
          <!-- Page Heading -->
              <%int countMsg = 0;
                for(int i=messageList.size()-1;i>=0;i--){
                  if(messageList.get(i).getPerpose().equals("homework")) {
                    countMsg++;
                  }
                }%>
              <h1 class="h3 mb-4 text-gray-800">Homeworks</h1>
              <%if(countMsg>0){%>
        ...
        <%for(int i=messageList.size()-1;i>=0;i--){%>
        <%if(messageList.get(i).getPerpose().equals("homework")){%>
        <form action="${pageContext.request.contextPath}/StudentMessageView" method="post" name="secureForm" id="reply_msg<%=i%>">
          <input type="hidden" name="id" value='<%=session.getAttribute("studentSessionID")%>'>
          <input type="hidden" name="studentName" value='<%=request.getAttribute("studentFullName")%>'>
        ...
        <%}%>
        <%}%>
        <form method="post" action="StudentMessages" id="delete_form">
          <input type="hidden" name="id" value='<%=session.getAttribute("studentSessionID")%>'>
          <input type="hidden" name="studentName" value='<%=request.getAttribute("studentFullName")%>'>
          <input type="hidden" name="messageType" value="homweork">
