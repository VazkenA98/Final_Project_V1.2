...
<% List<Teacher> teacherList = (List<Teacher>) request.getAttribute("departmentTeachers");%>
<!DOCTYPE html>
<html lang="en">
            ...
            <tr>
              ...
              <td>
                <div class="card2" style="background-color: #e6e6e6; padding: 1px;box-shadow: 10px 10px 5px grey;">
                  ...
                  <form action="${pageContext.request.contextPath}/DepartmentViewProfiles" method="post" name="secureForm" id="steacherProfile<%=i%>" >
                    <input type="hidden" name="id" value='<%=session.getAttribute("departmentSessionID")%>'>
                    <input type="hidden" name="departmentName" value="<%=request.getAttribute("departmentFullName")%>">
                    <input type="hidden" name="reciverCustemID" value="<%=teacherList.get(i).getCustem_ID()%>">
                  ...
                  <p><a class="dropdown-item" style="width: 93%;margin-left: 10px;  background-color: #4CAF50; border: none; color: white; border-radius: 25px;" onclick="document.getElementById('steacherProfile<%=i%>').submit();" href="#">Profile</a></p>
                </div>
              </td>
              <!-- /.container-fluid -->
              <%if(count==3){
