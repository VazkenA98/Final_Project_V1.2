...
<% List<Department> departmentList = (List<Department>) request.getAttribute("departmentsList");%>
<!DOCTYPE html>
<html lang="en">
          ...
          <tr>
            ...
            <td>
              <div class="card2" style="background-color: #e6e6e6; padding: 1px;box-shadow: 10px 10px 5px grey;">
                <%if(departmentList.get(i).getImg() == null){%>
                <img src="img/e1.jpg" alt="John" style="width: 299px;height: 230px;">
                <%}else{%>
                <img src="<%=request.getContextPath()%>/<%=departmentList.get(i).getImg()%>" alt="John" style="width: 299px;height: 230px;">
                <%}%>
                <h1 style="font-size: 28px"><b><%=departmentList.get(i).getName()+" "+departmentList.get(i).getSurname()%></b></h1>
                <a class="Phone"  href="tel:<%=departmentList.get(0).getPhone()%>"><%=departmentList.get(0).getPhone()%></a>
                <% if(departmentList.get(i).getOnline_status() == 0){%>
                <p style="color: #dc3545">Offline</p>
                <%}else{%>
                <p style="color: #1cc88a">Online</p>
                <%}%>
                <form action="${pageContext.request.contextPath}/DepartmentSendEmail" method="post" name="secureForm" id="sendEmail<%=i%>" >
                  <input type="hidden" name="id" value='<%=session.getAttribute("departmentSessionID")%>'>
                  <input type="hidden" name="process" value="check">
                  <input type="hidden" name="reciverEmail" value='<%=departmentList.get(i).getRecover_email()%>'>
                  <input type="hidden" name="departmentName" value="<%=request.getAttribute("departmentFullName")%>">
                  <input type="hidden" name="reciverEmail" value='<%=departmentList.get(i).getRecover_email()%>'>
                  <input type="hidden" name="reciverCustemID" value="<%=departmentList.get(i).getCustem_ID()%>">
                  <input type="hidden" name="msgType" value="msg">
                </form>


                <p><a class="dropdown-item" style="width: 93%;margin-left: 10px;  background-color: #4CAF50; border: none; color: white; border-radius: 25px;" onclick="document.getElementById('sendEmail<%=i%>').submit();" href="#">Send Message</a></p>
              </div>
            </td>
            <!-- /.container-fluid -->
            <%if(count==3){
