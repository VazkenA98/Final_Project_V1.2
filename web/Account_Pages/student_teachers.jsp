...
<% List<Teacher> teacherList = (List<Teacher>) request.getAttribute("studentTeachers");%>
<!DOCTYPE html>
<html lang="en">
              ...
              <% count ++;%>
                <td>
                  <div class="card2" style="background-color: #e6e6e6; padding: 1px;box-shadow: 10px 10px 5px grey;">
                      <%if(teacherList.get(i).getImg() == null){%>
                      <img src="img/e1.jpg" alt="John" style="width: 299px;height: 230px;">
                      <%}else{%>
                      <img src="<%=request.getContextPath()%>/<%=teacherList.get(i).getImg()%>" alt="John" style="width: 299px;height: 230px;">
                      <%}%>
                        <h1 style="font-size: 28px"><b><%=teacherList.get(i).getName()+" "+teacherList.get(i).getSurname()%></b></h1>
                        <p class="title"><%=teacherList.get(i).getSbjectTeachesToGroup()%></p>
                      <a class="Phone"  href="tel:<%=teacherList.get(0).getPhone()%>"><%=teacherList.get(0).getPhone()%></a>
                      <% if(teacherList.get(i).getOnline_status() == 0){%>
                      <p style="color: #dc3545">Offline</p>
                      <%}else{%>
                      <p style="color: #1cc88a">Online</p>
                      <%}%>
                    <form action="${pageContext.request.contextPath}/SendEmail" method="post" name="secureForm" id="sendEmailMsg<%=i%>" >
                      <input type="hidden" name="id" value='<%=session.getAttribute("studentSessionID")%>'>
                      <input type="hidden" name="process" value="check">
                      <input type="hidden" name="reciverEmail" value='<%=teacherList.get(i).getRecover_email()%>'>
                      <input type="hidden" name="studentName" value="<%=request.getAttribute("studentFullName")%>">
                      <input type="hidden" name="reciverCustemID" value="<%=teacherList.get(i).getCustem_ID()%>">
                      <input type="hidden" name="msgType" value="msg">
                    </form>
                    <form action="${pageContext.request.contextPath}/SendEmail" method="post" name="secureForm" id="sendEmailHome<%=i%>" >
                      <input type="hidden" name="id" value='<%=session.getAttribute("studentSessionID")%>'>
                      <input type="hidden" name="process" value="check">
                      <input type="hidden" name="reciverEmail" value='<%=teacherList.get(i).getRecover_email()%>'>
                      <input type="hidden" name="studentName" value="<%=request.getAttribute("studentFullName")%>">
                      <input type="hidden" name="reciverCustemID" value="<%=teacherList.get(i).getCustem_ID()%>">
                      <input type="hidden" name="msgType" value="homework">
                    </form>
                    <form action="${pageContext.request.contextPath}/StudentTeacherView" method="post" name="secureForm" id="viewStore<%=i%>" >
                      <input type="hidden" name="id" value='<%=session.getAttribute("studentSessionID")%>'>
                      <input type="hidden" name="studentName" value="<%=request.getAttribute("studentFullName")%>">
                      <input type="hidden" name="teacherID" value="<%=teacherList.get(i).getId()%>">
                      <input type="hidden" name="teacherName" value="<%=teacherList.get(i).getName()+" "+teacherList.get(i).getSurname()%>">
                    </form>
                      ...
                      <p><a class="dropdown-item" style="width: 93%;margin-left: 10px;  background-color: #4CAF50; border: none; color: white; border-radius: 25px;" onclick="document.getElementById('sendEmailHome<%=i%>').submit();" href="#">Send Homework</a></p>
                      <p><a class="dropdown-item" style="width: 93%;margin-left: 10px;  background-color: #4CAF50; border: none; color: white; border-radius: 25px;" onclick="document.getElementById('viewStore<%=i%>').submit();" href="#">View Store</a></p>
                    </div>
                </td>
                <!-- /.container-fluid -->
                  <%if(count==3){
