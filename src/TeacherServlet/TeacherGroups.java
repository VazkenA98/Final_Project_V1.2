...
@WebServlet("/TeacherGroup")
public class TeacherGroups extends HttpServlet {
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        try {
            teacherStudent(request,response);
    ...
    private void teacherStudent(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException, SQLException, ClassNotFoundException {
    ...
    private void setUpTeacherStudentsGroup(HttpServletRequest request, HttpServletResponse response) throws SQLException, ClassNotFoundException, IOException, ServletException {
        ...
        getTeacherGroups(request,response,Integer.parseInt(teacherID));

    }

    private void getTeacherGroups(HttpServletRequest request, HttpServletResponse response, int teacherID) throws SQLException, ServletException, IOException {
        TeacherDAO teacherDAO =  new TeacherDAO();
        List<Integer> groupsID = teacherDAO.getTeacherGroupsID(teacherID);
        List<Group> groups = new ArrayList<>();
        for (int i=0;i<groupsID.size();i++) {
            groups.add(teacherDAO.getTeacherGroups(groupsID.get(i)));
        }
        Set<String> nameSet = new HashSet<>();
        List<Group> groupDistinctByName = groups.stream()
                .filter(e -> nameSet.add(e.getName()))
                .collect(Collectors.toList());
        request.setAttribute("teacherGroups",groupDistinctByName);
        RequestDispatcher rd = null;
        rd = request.getRequestDispatcher("/Account_Pages/teacher_group.jsp");
