...
@WebServlet("/DepartmentGroupMailing")
@MultipartConfig(fileSizeThreshold=1024*1024*2, // 2MB
        maxFileSize=1024*1024*10,      // 10MB
        maxRequestSize=1024*1024*50)
public class DepartmentGroupMailing extends HttpServlet {
    private static final String SAVE_DIR = "uploadFilesStore";
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        try {
            departmentGroupMailing(request,response);
    ...
    private void departmentGroupMailing(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException, SQLException, ClassNotFoundException {
    ...
    private void checkProcess(HttpServletRequest request, HttpServletResponse response,String departmentID) throws IOException, ServletException, SQLException, ClassNotFoundException {
        ...
        if (process.equals("check")) {
            rd = request.getRequestDispatcher("/Account_Pages/department_group_messege_send.jsp");
            ...
            for (int i = 0;i<groupStudentsID.size();i++) {
                Integer msgID = mailSendingDAO.departmentSendMessage(groupStudentsID.get(i), departmentID, AES256.encrypt(subject), AES256.encrypt(content), msgType);
